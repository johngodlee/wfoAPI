...
- **User-specified API endpoint**: Use custom deployments of the WFO database 
...
The primary function in the package is `matchName()`, which takes a vector of taxonomic names and returns matched names from the WFO database.

```{r}
library(wfoAPI)

# Match a single species name
t(matchName("Burkea africana"))
```

The function returns a data frame with the following information:

* `taxon_name_raw` - Original name as in `x`
* `taxon_name_subm` - Name after optional sanitisation according to `sub_pattern`, `tolower`, `nonumber`
...
`matchName()` can process multiple names efficiently:
...
```{r}
matchName("Burkea africana", raw = TRUE)
```

When a submitted name is a synonym, the function automatically retrieves the currently accepted name:

```{r}
t(matchName("Lonchocarpus nelsii"))
```

## Selection algorithms

The WFO API performs server-side fuzzy matching to handle spelling variations and errors. Where a name is not uniquely matched a list of fuzzy-matched candidates is returned. The arguments provided to `matchName()` alter how these candidate names are handled.

If `interactive = TRUE`, you can manually select the correct candidate:

```{r eval=FALSE}
# NOT RUN
matchName("Ochna pul", interactive = TRUE)
```

If `preferAccepted = TRUE`, if only one candidate is an "accepted" name, this name is automatically selected.

```{r}
t(matchName("Strychnos pungens", interactive = TRUE, preferAccepted = TRUE))
```
...
```{r}
t(matchName("Brachystegia spic", interactive = FALSE, preferFuzzy = TRUE))
```

## Sanitising names

Other arguments to `matchName()` control how names are pre-processed before being sent to the WFO API. 
...
Like `matchName()` this function can also return raw values as a list.

## Extracting taxonomy information from a WFO ID

WFO IDs can also be used to query the WFO database using the `getName()` function. This function returns similar variables to the `matchName()` function.

```{r}
getName(c("wfo-0000455689", "wfo-0001049423", "wfo-4000011857"))
```
...
`wfo_cache_get()` points to `the$wfo_cache`. The cache contains two lists, one for the results of `matchName()` (`the$wfo_cache$matchName`) and one for the results of `getRank()` (`the$wfo_cache$getRank`).

To use cached names, set argument `useCache = TRUE` in `matchName()`. Similarly, you can disable API access and only use the cache, by setting argument `useAPI = FALSE`.

# Specifying versions of the WFO taxonomic backbone

`wfoVersion()` provides the most recent version of the WFO database as a "YYYY-MM" character string. `getRank()`, `matchName()` and `getName()` allow the user to optionally specify a version of the WFO database. Versions of the database are generally released in June (06) and December (12) each year, going back to June 2018 ("2018-06").

```{r}
wfo_version_vec <- paste0(as.character(seq(2021, 2025, 1)), "-06")

ex_rank <- matchName("Hiptage triacantha")$taxon_wfo_acc

sapply(wfo_version_vec, function(x) { 
  message(x)
  getName(ex_rank, wfo_version = x)
})
```

## Managing API load

`matchName()`, `getRank()`, and `getName()` have arguments to limit the rate of API calls, to avoid overloading the API.
...
The user can specify a custom API endpoint either before or after the package has loaded, either as an 'option' (`options("wfo.api_uri" = "<URL>")`) or an environment variable stored in `.Renviron` (`Sys.setenv(WFO_API_URI = "<URL>")`).
